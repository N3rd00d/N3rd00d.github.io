...
프로젝트 아레나의 경우, iOS의 최저사양을 아이폰 8+(3GB)로 선정하였고, 마지노선은 1.6GB로 잡았다.(경험상 최대 가용 메모리는 1.75GB 정도이다)
...
프로젝트 아레나의 경우, 심리스 월드에서 네비게이션 메모리가 1GB씩 잡혀 가용 메모리를 까먹는 주요 원인 중 하나였다. 모바일에서 굳이 필요없다면 월드 세팅으로 네비게이션 시스템을 비활성화하자.
...
# 프레임 최적화
프로젝트 아레나는 동남아시아에 소프트론칭을 함에, 그 시장은 주로 안드로이드 저사양 단말기를 사용하는 편이었다. 이에 저사양 단말기에서 안정적인 플레이를 할 수 있는 프레임을 확보하는 것이 팀의 선결 과제였는데, 이러한 프레임 최적화 작업은 참으로 어려운 것이라, 이를 토로해 보자면
...
다음은, Profiler 도구를 사용해 발견한 개발버전의 프로젝트 아레나의 프레임 부하 사례를 소개하고자 한다.
